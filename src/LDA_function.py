...
def my_lda_func(corpus, num_topics, id2word, random_state=10,  passes=1, num_words=10,
    ...
    alpha = np.array(
        [1.0 / num_topics for i in range(num_topics)], dtype=dtype)
    ...
    for pass_ in range(passes):
        ...
        for chunk_no, chunk in enumerate(all_chunks):
            ...
            if len(chunk) > 1:
                if verbose:
                    print(
                        f'performing inference on a chunk of {len(chunk) } documents')
            else:
                raise

            # Initialize the variational distribution q(theta|gamma) for the chunk
            gamma = rand.gamma(100., 1. / 100., (len(chunk),
                               num_topics)).astype(dtype, copy=False)
            ...
            for d, doc in enumerate(chunk):
                ...
                cts = np.fromiter((cnt for _, cnt in doc),
                                  dtype=dtype, count=len(doc))
                gammad = gamma[d, :]
            ...
            if len(chunk) > 1:
                if verbose:
                    print(
                        f"{converged}/{len(chunk)} documents converged within {iterations} iterations")
            ...
            diff = np.mean(
                np.abs(previous_Elogbeta.ravel() - current_Elogbeta.ravel()))
            if verbose:
